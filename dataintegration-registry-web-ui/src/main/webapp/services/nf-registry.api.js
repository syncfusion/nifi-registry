...
//var NfEnableSecurityComponent = require('nifi-registry/components/login/dialogs/nf-enable-security.js');
var ngMaterial = require('@angular/material');

var config = {
    urls: {
        currentUser: '/nifi-registry-api/access',
        syncfusionProviderStatus: '/nifi-registry-api/syncfusion/status',
        kerberos: '/nifi-registry-api/access/token/kerberos',
        isUMPConfiguredPath: '/nifi-registry-api/syncfusion/isumpconfigured',
        enableUmsUserAccess: '/nifi-registry-api/syncfusion/enable-ums-user-access',
        configureump:'/nifi-registry-api/syncfusion/configureump',
        decrypt: '/nifi-registry-api/syncfusion/decryptclientsecret',
        syncfusionToken: '/nifi-registry-api/syncfusion/token',
        syncfusionCurrentuser: '/nifi-registry-api/syncfusion/access',
        token: '/nifi-registry-api/access/token',
        accessStatus: '/nifi-registry-api/access',
        accessConfig: '/nifi-registry-api/access/config',
        configureUMP: '/nifi-registry-api/syncfusion/configureump',
        getnifidetails: '/nifi-registry-api/syncfusion/getnifidetails',
        getumpusers:  '/nifi-registry-api/syncfusion/getumpusers'  ,
        currentIdentityUser: '../nifi-registry-api/flow/current-user',
        getUMSApplications: '../nifi-registry-api/syncfusion/get-ums-apps',
        getmanagecredentials: '/nifi-registry-api/syncfusion/manage-credential',
        getDipUrl : '/nifi-registry-api/syncfusion/getDipUrl'
    }
};
 ...
 * @constructor
 */
function NfRegistryApi(nfStorage, http, fdsDialogService, router,matDialog) {
...
NfRegistryApi.prototype = {
                ...
                .catch(function (error) {
                    self.dialogService.openConfirm({
                        ...
                        acceptButtonColor: 'fds-warn'
                            ...
                            function (accept) {
                                if (accept) {
                                    self.router.navigateByUrl('/dataintegration-registry/explorer/grid-list/buckets/' + bucketId);
                                }
                            });
                    return rxjs.Observable.of(error);
                });
                ...
                .catch(function (error) {
                    self.dialogService.openConfirm({
                        ...
                        acceptButtonColor: 'fds-warn'
                            ...
                            function (accept) {
                                if (accept) {
                                    self.router.navigateByUrl('/dataintegration-registry/explorer/grid-list');
                                }
                            });
                    return rxjs.Observable.of(error);
                });
    ...
    ValidateCredentials: function (username, password, login) {
        var self = this;
       return this.http.post(config.urls.getmanagecredentials,{'username': username,'password':password,'operationaltype':login},{responseType: 'text'},headers)
                .map(function (response) {
                    var  result= response;
                    return result;
                })
        ...
        return this.http.get(config.urls.currentUser)
                ...
                .catch(function (error) {
                    ...
                    if (error.status === 401) {
                        ...
                        self.router.navigateByUrl('/dataintegration-registry/login');
                }
                    return rxjs.Observable.of({
                        resourcePermissions: {
                            anyTopLevelResource: {
    ...
    syncfusionTicketExchange: function (code) {
        var self = this;
        if (this.nfStorage.hasItem('jwt')) {
            return rxjs.Observable.of(self.nfStorage.getItem('jwt'));
        } else {
            return this.http.post(config.urls.syncfusionToken,code,{responseType: 'text'})
                    .map(function (jwt) {
                        // get the payload and store the token with the appropriate expiration
                        var token = self.nfStorage.getJwtPayload(jwt);
                        if (token) {
    ...
    {
        var self = this;
        return this.http.get(config.urls.syncfusionProviderStatus)
                .map(function (response) {
                    return response;
                })
                 .catch(function (error) {
    ...
    getUMSApplicationDetails: function (currentuser) {
        var self = this;
        return this.http.post(config.urls.getUMSApplications, currentuser, {responseType: 'text'})
                .map(function (response) {
    ...
    enableUMSUserAccess: function (baseUrl, clientId, clientSecret)
    {
        ...
        return this.http.post(config.urls.enableUmsUserAccess, {
            'baseUrl': baseUrl,
            'clientId': clientId,
      ...
      umsUserEnable: function (baseUrl, clientId, clientSecret)
    {
   ...
   //  var hostname = window.origin;
        ...
        return this.http.post(config.urls.configureump, {
            'hostname': hostname,
            'baseUrl': baseUrl,
            'clientId': clientId,
            'clientSecret': clientSecret,
            'admin': admin,
            'isSecured': isSecured},{responseType: 'text'}
        ,headers)
        .map(function (response) {
                ...
                .catch(function (error) {
                    self.dialogService.openConfirm({
                        title: 'Error',
                        message: "Check whether data integration agent is running or not",
                        acceptButton: 'Ok',
                        acceptButtonColor: 'fds-warn'
                    });
                    return rxjs.Observable.of(error);
     ...
     decrypt : function(clientSecret){
         var self = this;
         var secretKey;
        // var headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
        return this.http.post(config.urls.decrypt,clientSecret,{responseType: 'text'})
                .map(function (clientSecret) {
    ...
    getAdminUser: function(baseUrl, clientId, clientSecret){
        var self = this;
         var options = {
            headers: headers,
            responseType: 'text'
       ...
       // var headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
        return this.http.post(config.urls.getumpusers,{'baseUrl': baseUrl,'clientId':clientId,'clientSecret':clientSecret},headers)
                .map(function (response) {
    ...
    isUMPConfigured:function (baseUrl)
    {
        var self = this;
        return this.http.post(config.urls.isUMPConfiguredPath,baseUrl,{responseType: 'text'})
                .map(function (response) {
                    return response;
                })
                .catch(function (error) {
      ...
      getLogoutUmsUrl: function ()
    {
        var self = this;
        return this.http.get('../nifi-registry-api/syncfusion/getumslogouturl')
                .map(function (response) {
                    return response;
                })
                 .catch(function (error) {
...
//    {
//        var self = this;
//        return this.http.get('../nifi-registry-api/syncfusion/getumsproperties')
//                .map(function (response) {
//                    return response;
//                })
//                 .catch(function (error) {
...
//    },
    ...
    loadSyncfusionCurrentUser: function () {
        var self = this;
        // get the current user

        return this.http.get(config.urls.syncfusionCurrentuser)
                .map(function (response) {
                    return response;
                })
                .catch(function (error) {
       ...
       openEnableSecurityConfigDialog: function () {
        var self = this;
        var hostname = window.location.hostname;
        return this.http.post(config.urls.getnifidetails,hostname,{responseType: 'text'})
                .map(function (response) {
                    return response;
                })
                .catch(function (error) {
    ...
    getAdminUserList: function(baseUrl, clientId, clientSecret){
        var self = this;
         var options = {
            headers: headers,
            responseType: 'text'
    ...
    {
        var self = this;
        var token=self.nfStorage.getItem('jwt');
        var headers = new ngCommonHttp.HttpHeaders({
        ...
        return this.http.get('../nifi-registry-api/syncfusion/importumsusers',options)
                .map(function (response) {
                    return response;
                })
                 .catch(function (error) {
        ...
        launchConfigureUmp: function (baseUrl, clientId, clientSecret, admin, isSecured)
    {
        var self = this;
        var hostname = window.location.hostname;
        return this.http.post('/nifi-registry-api/syncfusion/configureump', {
            'hostname': hostname,
            'baseUrl': baseUrl,
            'clientId': clientId,
            'clientSecret': clientSecret,
...
showErrorDialog: function (error) {
        ...
        if (error.statusText.indexOf("Unknown Error") > -1) {
        ...
        self.dialogService.openConfirm({
            ...
            acceptButtonColor: 'fds-warn'
                ...
                function (accept) {
                    if (accept && isSessionExpired ) {
                         if(window.location.protocol === "http:") {
                          window.location.href='dataintegration-registry/login/dataintegration-login' ;
                        }
                        else{
    ...
    getDipUrl : function()  {
        var self = this;
        var hostname = window.location.hostname;
        return this.http.post(config.urls.getDipUrl,hostname,{responseType: 'text'})
                .map(function (response) {
                    return response;
                })
                .catch(function (error) {
    ...
    addDipDialog:function()  {
       var self = this;
        self.dialogService.openConfirm({
            title: 'Configuration Alert',
            message: "Please configure Data Integration Platform first after the service started. You can ensure service status in service manager.",
            acceptButton: 'Configure',
            acceptButtonColor: 'fds-warn',
        }).afterClosed().subscribe(
                function (accept) {
                    if (accept) {
                        self.getDipUrl().subscribe(function (response) {
                            if (response !== "")  {
                                const link = document.createElement('a');
                                link.target = '_blank';
                                var dipUrl = response.replace(/['"]+/g, '');
                                link.href = dipUrl + "/dataintegration/login";
