...
function NfRegistryCreateBucket(nfRegistryApi, fdsSnackBarService, nfRegistryService, matDialogRef) {
    ...
    $(document).ready(function () {
        ...
        this.keepDialogOpen = false;
        var self = this;
        $('#bucket-create-name').on('keyup', function (e) {
            var bucketName = $('#bucket-create-name').val();
            if (e.keyCode === 13) {
                nfRegistryApi.createBucket(bucketName).subscribe(function (bucket) {
                    if (!bucket.error) {
                        nfRegistryService.buckets.push(bucket);
